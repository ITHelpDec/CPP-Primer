...
private:
    ...
    // helper utility function to move instead of copy?
...
// similar to reallocate function, but using make_move_iterator
// there is, howver, a function available called uninitialised_move()...
...
void StrVec::reallocate()
{
    ...
    // move the elmeents using two special functions
    // std::uninitialized_copy() and std::make_move_operator()
...
int main()
{
    std::cout << "Creating a StrVec...\n";
    StrVec s1;
    s1.push_back("man");
    s1.push_back("bear");
    s1.push_back("pig");

    std::cout << std::endl; vprint(s1);

    std::cout << "\nMoving s1 into new StrVec (s2)...\n";
    StrVec s2(std::move(s1));

    std::cout << std::endl; vprint(s2);

    std::cout << "\nMove assigning s2 into yet another new StrVec (s3)...\n";
    StrVec s3;
    s3 = std::move(s2);

    std::cout << std::endl; vprint(s3);
