...
int main()
{
    ...
    // for (const auto &q : qvec) { print_total(std::cout, q, 20); }
    // std::cout << std::endl;
    
    // base class type for shared pointer vector
    std::vector<std::shared_ptr<Quote>> shared_Quote_vector;

    // add bulk quotes to vector
    std::cout << "Adding bulk orders to shared pointer vector...\n";
    shared_Quote_vector.push_back(std::make_shared<Bulk_quote>(bulk1));
    shared_Quote_vector.push_back(std::make_shared<Bulk_quote>(bulk2));
    shared_Quote_vector.push_back(std::make_shared<Bulk_quote>(bulk3));

    // sums show as they should compared to the last exercise
    std::cout << "Printing net_price(20) from three vectors...\n\n";
    for (const auto &spq : shared_Quote_vector) { print_total(std::cout, *spq, 20); }
    std::cout << std::endl;
